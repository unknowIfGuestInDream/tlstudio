 ...
 * </dl>
 */
public class Chips extends AbstractCustomCanvas {
	...
	private final List<ChipsCloseListener> closeListeners = new ArrayList<>();
		...
		addListener(SWT.MouseUp, e -> {
			...
			if (isClose) {
				final float dist = (float) Math.sqrt(
						(e.x - closeCenter.x) * (e.x - closeCenter.x) + (e.y - closeCenter.y) * (e.y - closeCenter.y));
				if (dist < CLOSE_CIRCLE_RAY) {
					final ChipsCloseEvent event = new ChipsCloseEvent(e);
					for (final ChipsCloseListener listener : closeListeners) {
	 ...
	 * @see SelectionEvent
	 */
	public void addCloseListener(final ChipsCloseListener listener) {
		checkWidget();
		if (listener == null) {
	 ...
	 */
	public void removeCloseListener(final ChipsCloseListener listener) {
		checkWidget();
		if (listener == null) {
			SWT.error(SWT.ERROR_NULL_ARGUMENT);
		}
