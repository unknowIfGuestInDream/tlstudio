 ...
 * issues notification when pressed and released. A menu can be attached to this
 * button
 * <dl>
 * <dt><b>Styles:</b></dt>
 * <dd>PUSH, TOGGLE</dd>
	...
	private void addPaintListener() {
		super.addPaintListener(new PaintListener() {
			...
			private void drawTriangle() {
				...
				e.gc.fillPolygon(new int[] { e.x + rect.width - 15 + deltaX, e.y + rect.height / 2 - 1 + deltaY,
						e.x + rect.width - 8 + deltaX, e.y + rect.height / 2 - 1 + deltaY,
						e.x + rect.width - 12 + deltaX, e.y + rect.height / 2 + 3 + deltaY });
			}
		});
	...
	private void addMouseDownListener() {
		super.addListener(SWT.MouseDown, new Listener() {
			...
			private boolean shouldShowMenu(final int x, final int y) {
				return x >= splitButtonAreaLeft && y >= splitButtonAreaTop && x <= splitButtonAreaRight
						&& y <= splitButtonAreaBottom;
			}
		});
	}
