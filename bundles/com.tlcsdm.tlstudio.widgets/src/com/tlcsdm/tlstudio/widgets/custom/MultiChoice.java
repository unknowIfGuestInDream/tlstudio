...
public class MultiChoice<T> extends Composite {
	...
	protected void validateEntry() {
		...
		if (fieldsInError.size() == 1) {
			messageToDisplay = String.format("The entry %s is invalid, please check it!", //
					fieldsInError.get(0));
		} else {
			...
			messageToDisplay = String.format("The entries %s are invalid, please check it!", //
					sb.toString());
		}
		getDisplay().asyncExec(() -> {
	...
	private void createPopup() {
		...
		if (showSelectUnselectAll) {
			...
			selectAllLink.setText("<a>Seleccionar todo</a>");
			...
			deselectAllLink.setText("<a>Deseleccionar todo</a>");
			deselectAllLink.addListener(SWT.Selection, e -> {
				deselectAll();
			});
		}

		this.scrolledComposite = new ScrolledComposite(parent,
