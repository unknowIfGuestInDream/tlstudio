...
class LauncherLabel extends Canvas {
	...
	private void paintControl(final PaintEvent event) {
		...
		if (animationStep != 0) {
			final float zoom = 1f + animationStep
					* (Math.max(extent.x, extent.y) - Math.max(image.getBounds().width, image.getBounds().height))
					/ MAX_NUMBER_OF_STEPS / 100f;
			...
			gc.drawImage(image, 0, 0, image.getBounds().width, image.getBounds().height, xZoomedImage, yZoomedImage,
					(int) (image.getBounds().width * zoom), (int) (image.getBounds().height * zoom));
