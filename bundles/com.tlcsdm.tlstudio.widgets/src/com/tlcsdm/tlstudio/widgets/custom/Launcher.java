...
public class Launcher extends Composite {
	 ...
	 * @param isSelected by default, if <code>true</code>, the background is the
	 *                   lightshadow. Otherwise, the background color is white
	 */
	private void changeColor(final int index, final boolean isSelected) {
		if (index != -1 && items.get(index).label != null) {
			final Color selectedItemColor = selectedItemBackgroundColor == null
					? getDisplay().getSystemColor(SWT.COLOR_WIDGET_LIGHT_SHADOW)
					: selectedItemBackgroundColor;
			final Color itemColor = itemBackgroundColor == null ? getDisplay().getSystemColor(SWT.COLOR_WHITE)
					: itemBackgroundColor;
		...
		getDisplay().timerExec(0, new Runnable() {
			...
			public void run() {
				if (label.incrementAnimation()) {
					...
					fireSelectionListenersEvent(Launcher.this, event, SWT.Selection);
				}
			}
		});
	...
	private boolean fireSelectionListenersEvent(final Control control, final Event sourceEvent, int type) {
	...
	/**
	 * Set the background color for the items. If <code>null</code>, the default
	 * value (white) is used
	 * 
	...
	/**
	 * @return the background color of the item when it is selected,
	 *         <code>null</code> if default value
	 * @exception SWTException
	 *                         <ul>
	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                         disposed</li>
	...
	/**
	 * @return <code>true</code> if the animation (and selection event) is fired
	 *         when one clicks
	 * @exception SWTException
	 *                         <ul>
	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	...
	/**
	 * @param singleClickSelection if true, the animation (and selection event) is
	 *                             fired on a single click. If false the selection
	 *                             is performed on a double click
	 * @exception SWTException
	 *                         <ul>
	 *                         <li>ERROR_WIDGET_DISPOSED - if the receiver has been
	 *                         disposed</li>
