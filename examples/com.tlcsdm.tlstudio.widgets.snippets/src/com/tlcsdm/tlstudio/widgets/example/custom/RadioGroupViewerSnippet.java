...
import com.tlcsdm.tlstudio.widgets.viewers.RadioGroupViewer;

public class RadioGroupViewerSnippet {
	...
	private static Shell createShell() {
		...
		final Text events = new Text(eventGroup, SWT.MULTI | SWT.READ_ONLY | SWT.V_SCROLL | SWT.H_SCROLL | SWT.BORDER);
		...
		removePerson.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				...
				if (!selection.isEmpty()) {
					Person person = (Person) selection.getFirstElement();
					if (MessageDialog.openConfirm(shell, "Remove person", "Remove person " + person.getName() + "?")) {
